...
$nombre = $apellido = $cedula = $telefono = $direccion = $correo = $fecha_nacimiento = "";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = $conn->prepare("SELECT id_paciente FROM paciente WHERE cedula = ?");
        $stmt->bind_param("s", $cedula);
        $stmt->execute();
        $stmt->store_result();

        if ($stmt->num_rows > 0)
            $errors[] = "Ya existe un paciente registrado con esa cédula.";
    ...
    if (empty($errors)) {
        $stmt = $conn->prepare("
            INSERT INTO paciente (nombre, apellido, cedula, telefono, direccion, correo, fecha_nacimiento)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        ");
