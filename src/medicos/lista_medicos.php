...
if (isset($_GET['eliminar'])) {
  if ($_SESSION['rol'] !== 'admin') {
    header("Location: /medicos/lista_medicos.php?msg_error=Acción+no+permitida.");
    exit;
  }
  $id = intval($_GET['eliminar']);

  $check = $conn->prepare("SELECT COUNT(*) AS total FROM cita WHERE id_medico = ? AND estado IN ('Pendiente', 'Confirmada')");
  ...
  if ($total > 0) {
    header("Location: /medicos/lista_medicos.php?msg_error=No+se+puede+eliminar:+el+médico+tiene+{$total}+cita(s)+activa(s).");
    exit;
  }
  ...
  header("Location: /medicos/lista_medicos.php?msg=Médico+eliminado+correctamente.");
  exit;
}
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["actualizar"])) {
  ...
  $stmt = $conn->prepare("UPDATE medico SET nombre=?, apellido=?, cedula=?, telefono=?, correo=?, id_especialidad=? WHERE id_medico=?");
  $stmt->bind_param("sssssii", $nombre, $apellido, $cedula, $telefono, $correo, $id_especialidad, $id);
  $stmt->execute();
  $stmt->close();

  header("Location: /medicos/lista_medicos.php?msg=Médico+actualizado+correctamente.");
  exit;
}
...
// 2. RECIÉN AQUÍ el HTML
include __DIR__ . "/../includes/plantilla.php";
?>

<!-- aquí va todo el HTML que ya tienes, sin cambios -->
  ...
  <div style="padding: 30px;">
    ...
    <?php if ($editando && $medico): ?>
      <div class="alert alert-info mb-4">
        ...
        <form method="POST" class="mt-3">
          ...
          <a href="/medicos/lista_medicos.php" class="btn btn-secondary ms-2">Cancelar</a>
        </form>
      </div>
    <?php endif; ?>
              ...
              <?php if ($_SESSION['rol'] === 'admin'): ?>
                <td style="text-align:center; white-space:nowrap;">
                  <a class="btn btn-sm btn-primary"
                    href="/medicos/editar_medico.php?id=<?= $row['id_medico'] ?>" title="Editar">
                    <i class="bi bi-pencil"></i>
                  </a>
                  <a class="btn btn-sm btn-danger"
                    href="/medicos/lista_medicos.php?eliminar=<?= $row['id_medico'] ?>"
                    title="Eliminar"
    ...
    <?php if ($_SESSION['rol'] === 'admin'): ?>
      <div class="d-flex justify-content-end mt-4">
        <a href="/medicos/registrar_medico.php" class="btn btn-success">
