...
<?php
soloAdmin(); // Solo administradores pueden gestionar usuarios del sistema
...
if (isset($_GET['eliminar'])) {
    ...
    // Evitar que se elimine el propio usuario activo
    if ($id === intval($_SESSION['id_usuario'])) {
        header("Location: lista_empleados.php?msg_error=No+puedes+eliminar+tu+propio+usuario.");
        exit;
    }

    // Evitar eliminar el único administrador activo
    $checkAdmin = $conn->query(
        "SELECT COUNT(*) AS total FROM usuario WHERE rol = 'admin' AND estado = 1"
    );
    ...
    if ($rolUsuario === 'admin' && $totalAdmins <= 1) {
        header("Location: lista_empleados.php?msg_error=No+puedes+eliminar+el+único+administrador+activo.");
        exit;
    }

    // Eliminar de usuario (empleado se mantiene si se quiere conservar historial)
    $stmt = $conn->prepare("DELETE FROM usuario WHERE id_usuario = ?");
    $stmt->bind_param("i", $id);
    $stmt->execute();
    $stmt->close();

    header("Location: lista_empleados.php?msg=Usuario+eliminado+correctamente.");
    exit;
}
...
if (isset($_GET['toggle'])) {
    ...
    if ($id === intval($_SESSION['id_usuario'])) {
        header("Location: lista_empleados.php?msg_error=No+puedes+desactivar+tu+propio+usuario.");
        exit;
    }
    ...
    if ($info['rol'] === 'admin' && $info['estado'] == 1 && $totalAdmins <= 1) {
        header("Location: lista_empleados.php?msg_error=No+puedes+desactivar+el+único+administrador+activo.");
        exit;
    }
    ...
    header("Location: lista_empleados.php?msg=$msg");
    exit;
}

// ==========================================
// LISTAR USUARIOS (JOIN con empleado para teléfono/correo)
// ==========================================
$result = $conn->query("
          ...
          <tr class="<?= $row['estado'] == 0 ? 'table-secondary text-muted' : '' ?>">
            ...
            <td style="text-align:center; white-space:nowrap;">
              ...
              <a class="btn btn-sm <?= $row['estado'] == 1 ? 'btn-warning' : 'btn-success' ?>"
                 href="lista_empleados.php?toggle=<?= $row['id_usuario'] ?>"
              ...
              <a class="btn btn-sm btn-danger"
                 href="lista_empleados.php?eliminar=<?= $row['id_usuario'] ?>"
                 title="Eliminar"
